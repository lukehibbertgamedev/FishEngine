...
namespace Fish {
	...
	namespace Loader {
		...
		std::optional<AllocatedImage> load_image(FishVulkanEngine* engine, fastgltf::Asset& asset, fastgltf::Image& image)
		{
			...
			std::visit(fastgltf::visitor{ [](auto& arg) {}, [&](fastgltf::sources::URI& filePath) {
				...
				if (data) {
					...
					std::visit(fastgltf::visitor { [](auto& arg) {}, [&](fastgltf::sources::Vector& vector) {
		...
		std::optional<std::shared_ptr<LoadedGLTF>> loadGltf(FishVulkanEngine* engine, std::string_view filePath)
		{
			...
			// 4. Temporary arrays to hold our mesh structures loaded by GLTF data.
			std::vector<std::shared_ptr<MeshAsset>> meshes;
			std::vector<std::shared_ptr<Node>> nodes;
			std::vector<AllocatedImage> images;
			std::vector<std::shared_ptr<GLTFMaterial>> materials;

			// 5. Load any/all textures.
			...
			// 6. Create buffer to hold the material data
			int data_index = 0;
			file.materialDataBuffer = engine->create_buffer13(sizeof(GLTFMetallic_Roughness::MaterialConstants) * gltf.materials.size(), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
			GLTFMetallic_Roughness::MaterialConstants* sceneMaterialConstants = (GLTFMetallic_Roughness::MaterialConstants*)file.materialDataBuffer.info.pMappedData;

			// 7. Loop to load materials
			...
			// 8. Loading meshes
			// Note: use the same vectors for all meshes so that the memory doesnt reallocate as often
			std::vector<uint32_t> indices;
			std::vector<Vertex> vertices;
			...
			// 10. run loop again to setup transform hierarchy
